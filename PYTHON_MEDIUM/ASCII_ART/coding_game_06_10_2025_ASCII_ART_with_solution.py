...
h = int(input())
t = input().lower()
my_dict = dict([(x,[]) for x in string.ascii_lowercase + "?"])
for i in range(h):
    row = input()
    for y,k in zip(range(0, len(row), l), my_dict):
        my_dict[k].append(row[y:y+l])
...
for level in range(h):
    ...
    for letter in t:
        if letter in string.ascii_lowercase:
            one_level.append(my_dict[letter][level])
        else:
            one_level.append(my_dict["?"][level])
    print("".join(one_level))
